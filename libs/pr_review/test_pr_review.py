...
def test_resolve_thread_requires_thread_id(client: FlaskClient) -> None:
    resp = client.post(f"/api/pr/{_REPO}/1/thread/resolve", json={})
    assert resp.status_code == 400
    assert "thread_id" in resp.get_json()["error"]


def test_unresolve_thread_requires_thread_id(client: FlaskClient) -> None:
    resp = client.post(f"/api/pr/{_REPO}/1/thread/unresolve", json={})
    assert resp.status_code == 400
    assert "thread_id" in resp.get_json()["error"]


def test_set_draft_requires_draft_field(client: FlaskClient) -> None:
    resp = client.post(f"/api/pr/{_REPO}/1/draft", json={})
    assert resp.status_code == 400
