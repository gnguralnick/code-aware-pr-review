...
@app.route("/api/pr/<owner>/<repo>/<int:number>/thread/resolve", methods=["POST"])
def api_resolve_thread(owner: str, repo: str, number: int) -> Response:
    """Mark a review-comment thread resolved (GraphQL)."""
    thread_id = (request.get_json(silent=True) or {}).get("thread_id", "")
    if not thread_id:
        return _err("thread_id is required", status=400)
    try:
        return jsonify(github.resolve_review_thread(thread_id))
    except github.GitHubError as exc:
        return _err(str(exc))


@app.route("/api/pr/<owner>/<repo>/<int:number>/thread/unresolve", methods=["POST"])
def api_unresolve_thread(owner: str, repo: str, number: int) -> Response:
    """Reopen a resolved review-comment thread (GraphQL)."""
    thread_id = (request.get_json(silent=True) or {}).get("thread_id", "")
    if not thread_id:
        return _err("thread_id is required", status=400)
    try:
        return jsonify(github.unresolve_review_thread(thread_id))
    except github.GitHubError as exc:
        return _err(str(exc))


@app.route("/api/pr/<owner>/<repo>/<int:number>/draft", methods=["POST"])
def api_set_draft(owner: str, repo: str, number: int) -> Response:
    """Flip a PR between draft and ready-for-review (GraphQL).

    ``draft: true`` converts to draft, ``draft: false`` marks ready. The PR's
    GraphQL node id is resolved server-side, so the client never sends it.
    """
    payload = request.get_json(silent=True) or {}
    if "draft" not in payload:
        return _err("draft (true or false) is required", status=400)
    is_draft = bool(payload["draft"])
    full = f"{owner}/{repo}"
    try:
        node_id = github.get_pr_node_id(full, number)
        result = github.convert_pr_to_draft(node_id) if is_draft else github.mark_pr_ready(node_id)
