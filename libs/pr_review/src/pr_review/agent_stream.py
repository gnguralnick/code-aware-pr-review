...
def _agent_env() -> dict:
    ...
    # Disable the imbue-code-guardian reviewer Stop hook for this child. That hook
    # is installed at *user* scope, so it loads even though we pass
    # `--setting-sources user`, and it reads `.reviewer/settings.json` relative to
    # cwd -- which, for prepare, is the reviewed checkout whose settings enable the
    # hook inside an mngr workspace (MNGR_AGENT_STATE_DIR set). It would then run
    # its review/CI gates when our install-only child tries to stop and block it
    # with `exit 2`. The hook takes no proxy-child marker, but its config layer
    # honors a per-process `CODE_GUARDIAN_<KEY>` env override (highest precedence);
    # a falsey `enabled_when` makes the orchestrator skip regardless of any repo's
    # `.reviewer` config. (The user disabling it in /code's own `.reviewer` only
    # covered the main repo root, not the checkouts this child runs inside.)
    env["CODE_GUARDIAN_STOP_HOOK__ENABLED_WHEN"] = "false"
    return env
