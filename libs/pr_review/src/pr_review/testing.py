...
class GraphQLFakeCurl(NamedTuple):
    """A body-aware stand-in for ``github._curl`` for GraphQL calls.

    Every GraphQL request hits the same ``/graphql`` URL, so :class:`FakeCurl`'s
    URL-substring routing can't tell operations apart. This dispatches instead on
    a substring of the GraphQL query/mutation text in the ``-d`` payload (e.g. the
    operation name ``resolveReviewThread``); the longest matching needle wins.
    ``payloads`` records each request's parsed ``{query, variables}`` so mutation
    tests can assert on the exact operation and variables sent. The HTTP-status
    marker is always appended (GraphQL requests are POSTs), so ``gh_graphql`` can
    parse the status just as it does for REST writes.
    """

    responses: tuple[tuple[str, bytes], ...]
    payloads: list[dict]
    status: int
    ...
    def __call__(self, args: list[str]) -> bytes:
        ...
        matches = [(needle, body) for needle, body in self.responses if needle in query]
        if not matches:
            raise AssertionError(f"GraphQLFakeCurl: no response matched query {query!r}")
        body = max(matches, key=lambda item: len(item[0]))[1]
        return body + (github._STATUS_MARKER + str(self.status)).encode()


def graphql_route(responses: dict[str, object], status: int = 200) -> GraphQLFakeCurl:
    """Build a :class:`GraphQLFakeCurl`. Keys are operation-name substrings; each
    value is JSON-encoded -- wrap it as ``{"data": {...}}`` (or ``{"errors": [...]}``
    to exercise the error path) yourself."""
    return GraphQLFakeCurl(
        responses=tuple((k, json.dumps(v).encode()) for k, v in responses.items()),
        payloads=[],
        status=status,
    )
...
def seed_repo_cache(repo: str, sha: str, files: dict[str, str]) -> Path:
    ...
    dest = github.REPO_CACHE / github.repo_slug(repo) / sha
...
def seed_prepared_state(
    ...
    ``prepare._publish`` to capture and for reuse/auto-enable to materialize. When
    ``python_venv`` is set, also seeds a stand-in venv at ``.pr-review-prep/venv``
    (a ``bin/python`` stub) and records ``python_venv`` in the status/result.
    """
    venv_rel = f"{prepare.PREP_DIRNAME}/venv" if python_venv else None
    status = {
        "state": "ready",
        "package_manager": package_manager,
        "roots": roots,
        "typescript_dir": prepare.PREP_DIRNAME,
        "python_venv": venv_rel,
    }
    prepare._write_status(tree, status)
