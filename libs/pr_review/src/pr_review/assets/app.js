...
// ===================================================================
// PR ACTIONS -- close / reopen / merge, and draft <-> ready-for-review, shared
// by the detail page and the home-page right-click menu. Marking a draft ready
// (or converting a ready PR back to a draft) goes through GitHub's GraphQL API
// via POST .../draft.
// ===================================================================
...
// Show a pending state on `btn` while an async action runs: disable it and swap
// its label to `pendingLabel`. These GraphQL actions take a network round-trip,
// so without this the button looks inert after a click. On success the caller's
// view usually re-renders the button away; if it's still in the DOM (e.g. the
// action failed or was cancelled), restore its original label and enabled state.
async function withButtonLoading(btn, pendingLabel, fn) {
  if (!btn) return fn();
  ...
  try {
    ...
    if (btn.isConnected) { btn.disabled = false; btn.textContent = prevText; }
  }
}

// Flip a PR between draft and ready-for-review (GraphQL, behind a confirm).
// Works from both the detail page and the home-page menu: updates the in-memory
// caches, the list, and -- if the PR is open -- the detail header in place.
async function actionSetDraft(repo, number, toDraft) {
  const ok = await confirmDialog({
    title: toDraft ? `Convert #${number} to draft?` : `Mark #${number} ready for review?`,
    message: toDraft
      ? "This converts the pull request back to a draft on GitHub. Reviewers won't be notified until it's marked ready again."
      : "This marks the pull request ready for review on GitHub, so reviewers are requested and it can be merged.",
    confirmLabel: toDraft ? "Convert to draft" : "Mark ready",
  });
  if (!ok) return false;
  try {
    await api2(`api/pr/${repo}/${number}/draft`, { draft: toDraft });
  } catch (e) { flashNote("Couldn't update: " + e.message); return false; }
  const newState = toDraft ? "draft" : "ready";
  const st = STATUS_CACHE[keyFor(repo, number)];
  if (st) st.state = newState;
  const pr = findPR(repo, number);
  if (pr) pr.state = newState;
  if (DETAIL && DETAIL.pr.repo === repo && DETAIL.pr.number === number) {
    DETAIL.pr.state = newState;
    refreshHeaderActions();
  }
  renderResults();
  updateChipCounts();
  flashNote(toDraft ? `#${number} converted to draft` : `#${number} marked ready for review`);
  return true;
}
...
function openRowMenu(e, repo, number) {
  ...
  if (isDraft) {
    items.push({ label: "Mark ready for review", fn: () => actionSetDraft(repo, number, false) });
  } else {
    const blocked = mergeBlockedReason(st);
    items.push({ label: "Merge…", disabled: !!blocked, title: blocked || "", fn: () => actionMerge(repo, number, st) });
    items.push({ label: "Convert to draft", fn: () => actionSetDraft(repo, number, true) });
  }
...
// The draft/open state pill (carries id="stateBadge" so it can be swapped in
// place after a draft <-> ready flip). Shared by statusBadges and the header
// refresh so the two never drift.
function stateBadgeHTML(pr) {
  return pr.state === "draft"
    ? '<span class="statusbadge" id="stateBadge"><span class="dot idle"></span>Draft</span>'
    : '<span class="statusbadge" id="stateBadge"><span class="dot ok"></span>Open</span>';
}
...
function statusBadges(pr) {
  ...
  return stateBadgeHTML(pr) + ci + rev + conflict;
}

function detailActionsHTML(pr) {
  const parts = [];
  if (pr.state === "draft") {
    parts.push('<button class="btn primary sm" id="actReady">Mark ready for review</button>');
  } else {
    ...
    parts.push('<button class="btn ghost sm" id="actDraft">Convert to draft</button>');
  }
...
function bindDetailActions(pr) {
  ...
  const ready = document.getElementById("actReady");
  if (ready) ready.addEventListener("click", () => withButtonLoading(ready, "Marking ready…", () => actionSetDraft(pr.repo, pr.number, false)));
  const draft = document.getElementById("actDraft");
  if (draft) draft.addEventListener("click", () => withButtonLoading(draft, "Converting…", () => actionSetDraft(pr.repo, pr.number, true)));
...
// After a draft <-> ready flip, refresh just the header's action buttons and the
// state badge in place (re-rendering the whole shell would drop the open file).
function refreshHeaderActions() {
  if (!DETAIL) return;
  const pr = DETAIL.pr;
  const actions = document.querySelector(".detail-actions");
  if (actions) { actions.outerHTML = detailActionsHTML(pr); bindDetailActions(pr); }
  const badge = document.getElementById("stateBadge");
  if (badge) badge.outerHTML = stateBadgeHTML(pr);
}
...
// A subtle pill in the detail header showing whether this repo has rich types
// (real type resolution for JavaScript/TypeScript and Python, including
// third-party libraries), with a single opt-in that prepares both languages.
let TYPES_POLL = null;
...
async function refreshTypesPill() {
  ...
  if (state === "ready") {
    ...
    pill.title = "Real type resolution (JavaScript/TypeScript and Python) is active for this repo. Click to rebuild or clear.";
    ...
    pill.title = "Basic syntax types. Enable rich types to install this repo's dependencies (JavaScript/TypeScript and Python) and resolve real types, including third-party libraries.";
...
async function enableRichTypes(isRetry) {
  ...
  const model = await confirmDialog({
    ...
    message: "This launches an agent that installs this repo's dependencies (running their install scripts) for both JavaScript/TypeScript and Python, so real types -- including from third-party libraries -- resolve on hover and go-to-definition. It can take a few minutes and uses your Claude usage.",
    confirmLabel: "Enable",
...
function renderConversation() {
  ...
  pane.querySelectorAll(".ct-resolve").forEach(btn => btn.addEventListener("click", () => {
    const resolve = btn.dataset.resolved !== "1";
    withButtonLoading(btn, resolve ? "Resolving…" : "Reopening…", () => setThreadResolved(btn.dataset.thread, resolve));
  }));
}
...
// Resolve or reopen a review-comment thread (GraphQL), then refresh both the
// conversation pane and the in-diff view zones so the new state shows everywhere.
async function setThreadResolved(threadId, resolve) {
  const action = resolve ? "resolve" : "unresolve";
  try {
    await api2(`api/pr/${repoPath()}/${DETAIL.pr.number}/thread/${action}`, { thread_id: threadId });
  } catch (e) { flashNote("Couldn't " + action + " thread: " + e.message); return; }
  flashNote(resolve ? "Thread resolved" : "Thread reopened");
...
function codeThreadCard(thread) {
  ...
  // The resolve/unresolve control only appears when we know the thread's GraphQL
  // id (present once thread state has been fetched).
  const resolveBtn = root.thread_id
    ? `<button class="btn ghost sm ct-resolve" data-thread="${attr(root.thread_id)}" data-resolved="${resolved ? "1" : "0"}">${resolved ? "Unresolve" : "Resolve"}</button>`
    : "";
  return `<div class="cmt-card codethread ${resolved ? "resolved" : ""}" data-path="${attr(root.path)}" data-line="${root.line || ""}" data-side="${attr(root.side || "RIGHT")}">
    <div class="ct-loc">
      <span class="ct-file mono" title="${attr(root.path)}">${loc}</span>
      ${resolved ? '<span class="pill resolved">resolved</span>' : ""}
...
// Count published review comments (code-localized, incl. replies) per file path,
// so the changed-files list can show how many comments each file carries.
function commentCountByPath() {
  const conv = DETAIL.conversation || { review_comments: [] };
  const counts = {};
  for (const c of (conv.review_comments || [])) {
    if (c.path) counts[c.path] = (counts[c.path] || 0) + 1;
  }
  return counts;
}

function changedFilesHTML() {
  const counts = commentCountByPath();
  return DETAIL.files.map(f => {
    ...
    const cmt = n
      ? `<span class="fcomments" title="${n} comment${n === 1 ? "" : "s"} on this file">&#128172; ${n}</span>`
      : "";
...
function threadNode(existing, pending) {
  ...
  // Resolve/unresolve control for a published thread (identified by its GraphQL
  // id); pending-only lines have no thread yet, so they get no control.
  const rootComment = existing.find(c => c.thread_id);
  if (rootComment) {
    const resolved = !!rootComment.is_resolved;
    if (resolved) wrap.classList.add("resolved");
    const bar = document.createElement("div");
    bar.className = "cz-resolve";
    bar.innerHTML = `${resolved ? '<span class="pill resolved">resolved</span>' : ""}<span class="spacer" style="flex:1"></span>`;
    const btn = document.createElement("button");
    btn.className = "btn ghost sm";
    btn.textContent = resolved ? "Unresolve" : "Resolve";
    btn.onclick = () => withButtonLoading(btn, resolved ? "Reopening…" : "Resolving…", () => setThreadResolved(rootComment.thread_id, !resolved));
    bar.appendChild(btn);
...
async function selectChangedFile(path) {
  ...
  // Trust the content-derived flag (the server read the file and checked for a
  // null byte), not the changed-files list hint -- GitHub omits `patch` for a
  // pure rename too, which the list hint can't distinguish from binary.
  if (content.binary) {
