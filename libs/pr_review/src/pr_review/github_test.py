...
# --- gh_graphql transport ---


def test_gh_graphql_returns_data_object() -> None:
    curl = graphql_route({"viewer": {"data": {"viewer": {"login": "octocat"}}}})
    assert github.gh_graphql("query { viewer { login } }", None, curl) == {"viewer": {"login": "octocat"}}


def test_gh_graphql_sends_query_and_variables() -> None:
    curl = graphql_route({"viewer": {"data": {"viewer": {"login": "octocat"}}}})
    github.gh_graphql("query { viewer { login } }", {"a": 1}, curl)
    assert curl.payloads[0] == {"query": "query { viewer { login } }", "variables": {"a": 1}}


def test_gh_graphql_raises_on_errors_array_with_messages() -> None:
    curl = graphql_route({"viewer": {"errors": [{"message": "Bad credentials"}]}})
    with pytest.raises(github.GitHubError, match="Bad credentials"):
        github.gh_graphql("query { viewer { login } }", None, curl)


def test_gh_graphql_raises_on_non_json() -> None:
    curl = make_curl({"graphql": b"<html>nope</html>"})
    with pytest.raises(github.GitHubError, match="non-JSON"):
        github.gh_graphql("query { viewer }", None, curl)


def test_gh_graphql_raises_on_http_error_status() -> None:
    curl = make_curl({"graphql": b"{}"}, status=502)
    with pytest.raises(github.GitHubError, match="502"):
        github.gh_graphql("query { viewer }", None, curl)


# --- review threads (resolve / unresolve) ---


def test_list_review_threads_parses_nodes_and_comment_ids() -> None:
    data = {
        "data": {
            "repository": {
                "pullRequest": {
                    "reviewThreads": {
                        "nodes": [
                            {
                                "id": "T1", "isResolved": True, "isCollapsed": True,
                                "comments": {"nodes": [{"databaseId": 11, "path": "a.py"}, {"databaseId": 12, "path": "a.py"}]},
                            },
                            {
                                "id": "T2", "isResolved": False, "isCollapsed": False,
                                "comments": {"nodes": [{"databaseId": 13, "path": "b.py"}]},
                            },
                        ]
                    }
                }
    ...
    curl = graphql_route({"reviewThreads": data})
    threads = github.list_review_threads("octocat/hello", 5, curl)
    assert threads == [
        {"id": "T1", "is_resolved": True, "comment_ids": [11, 12]},
        {"id": "T2", "is_resolved": False, "comment_ids": [13]},
    ]
    assert curl.payloads[0]["variables"] == {"owner": "octocat", "name": "hello", "number": 5}


def test_list_review_threads_empty_when_no_pull_request() -> None:
    curl = graphql_route({"reviewThreads": {"data": {"repository": {"pullRequest": None}}}})
    assert github.list_review_threads("o/r", 5, curl) == []


def test_resolve_review_thread_sends_mutation_and_variables() -> None:
    curl = graphql_route(
        {"resolveReviewThread": {"data": {"resolveReviewThread": {"thread": {"id": "T1", "isResolved": True}}}}}
    )
    assert github.resolve_review_thread("T1", curl) == {"id": "T1", "isResolved": True}
    assert curl.payloads[0]["variables"] == {"threadId": "T1"}
    assert "resolveReviewThread" in curl.payloads[0]["query"]


def test_unresolve_review_thread_sends_mutation_and_variables() -> None:
    curl = graphql_route(
        {"unresolveReviewThread": {"data": {"unresolveReviewThread": {"thread": {"id": "T1", "isResolved": False}}}}}
    )
    assert github.unresolve_review_thread("T1", curl) == {"id": "T1", "isResolved": False}
    assert curl.payloads[0]["variables"] == {"threadId": "T1"}
    assert "unresolveReviewThread" in curl.payloads[0]["query"]


# --- draft <-> ready-for-review ---


def test_mark_pr_ready_sends_mutation_and_variables() -> None:
    curl = graphql_route(
        {"markPullRequestReadyForReview": {"data": {"markPullRequestReadyForReview": {"pullRequest": {"id": "PR1", "isDraft": False}}}}}
    )
    assert github.mark_pr_ready("PR1", curl) == {"id": "PR1", "isDraft": False}
    assert curl.payloads[0]["variables"] == {"pullRequestId": "PR1"}
    assert "markPullRequestReadyForReview" in curl.payloads[0]["query"]


def test_convert_pr_to_draft_sends_mutation_and_variables() -> None:
    curl = graphql_route(
        {"convertPullRequestToDraft": {"data": {"convertPullRequestToDraft": {"pullRequest": {"id": "PR1", "isDraft": True}}}}}
    )
    assert github.convert_pr_to_draft("PR1", curl) == {"id": "PR1", "isDraft": True}
    assert curl.payloads[0]["variables"] == {"pullRequestId": "PR1"}
    assert "convertPullRequestToDraft" in curl.payloads[0]["query"]


def test_get_pr_node_id_uses_single_pr_get() -> None:
    curl = make_curl({"/pulls/5": json.dumps({"node_id": "PR_node_9"}).encode()})
    assert github.get_pr_node_id("octocat/hello", 5, curl) == "PR_node_9"
    # One lightweight REST call -- the PR fetch -- not enrich_status's several.
    assert len(curl.calls) == 1
    assert curl.calls[0][-1].endswith("/repos/octocat/hello/pulls/5")


def test_get_pr_node_id_raises_when_missing() -> None:
    curl = make_curl({"/pulls/5": b"{}"})
    with pytest.raises(github.GitHubError, match="node id"):
        github.get_pr_node_id("octocat/hello", 5, curl)
...
def test_list_changed_files_maps_entries_and_detects_binary_and_rename() -> None:
    page = [
        ...
        {"filename": "img.png", "status": "modified"},  # modified, no patch -> binary
        {"filename": "new.py", "previous_filename": "old.py", "status": "renamed", "patch": "@@"},
        # Pure rename with no content change: GitHub omits the patch, but this is a
        # text file that moved, NOT binary.
        {"filename": "moved.py", "previous_filename": "was.py", "status": "renamed"},
    ]
    curl = make_curl({"page=1": json.dumps(page).encode(), "page=2": b"[]"})
    files = github.list_changed_files("o/r", 1, curl)
    assert [f["path"] for f in files] == ["a.py", "img.png", "new.py", "moved.py"]
    assert files[1]["is_binary"] is True  # modified + no patch
    assert files[0]["is_binary"] is False  # modified + patch
    assert files[2]["previous_path"] == "old.py"
    assert files[3]["is_binary"] is False  # renamed + no patch is not binary
...
def test_get_conversation_maps_and_filters_pending_reviews() -> None:
    ...
    # One GraphQL review thread covering comment 4 (resolved); comment 5 has no
    # matching thread. The graphql URL is routed by its "graphql" substring.
    threads_data = {
        "data": {
            "repository": {
                "pullRequest": {
                    "reviewThreads": {
                        "nodes": [
                            {
                                "id": "TR4", "isResolved": True, "isCollapsed": False,
                                "comments": {"nodes": [{"databaseId": 4, "path": "f.py"}]},
                            }
                        ]
                    }
                }
    ...
    curl = make_curl(
        {
            ...
            "graphql": json.dumps(threads_data).encode(),
        }
    )
    ...
    # Comment 4 maps to its resolved GraphQL thread; comment 5 has no thread.
    assert out["review_comments"][0]["thread_id"] == "TR4"
    assert out["review_comments"][0]["is_resolved"] is True
    assert out["review_comments"][1]["thread_id"] is None
    assert out["review_comments"][1]["is_resolved"] is False


def test_get_conversation_survives_graphql_thread_failure() -> None:
    # When the GraphQL thread-state call errors, the comments still render -- just
    # without resolution state (thread_id None, is_resolved False).
    review_comments = [
        {"id": 4, "user": {"login": "d"}, "path": "f.py", "line": 12, "body": "nit", "created_at": "t"},
    ]
    curl = make_curl(
        {
            "/issues/1/comments": b"[]",
            "/pulls/1/reviews": b"[]",
            "/pulls/1/comments": json.dumps(review_comments).encode(),
            "graphql": json.dumps({"errors": [{"message": "missing scope"}]}).encode(),
        }
    )
    out = github.get_conversation("o/r", 1, curl)
    assert out["review_comments"][0]["thread_id"] is None
    assert out["review_comments"][0]["is_resolved"] is False
